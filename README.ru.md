...
**Ловушка:** в ReactLynx код в **модульной области выполняется на ОБОИХ потоках**.
Поэтому вот так падает всегда, своей главнопоточной половиной:

```ts
// ❌ верхний уровень файла — выполняется и на главном потоке
const key = getRandomValues(new Uint8Array(32))
```

Зови изнутри компонента, из эффекта или из кода, который заведомо идёт
на фоновом потоке.
